...
def main() -> None:
    ...
    parser: object = argparse.ArgumentParser(prog='dnsbrute',
                                            description='Brute DNS enumerator',
                                            epilog='Enumerates the given DNS using an internal wordlist')

    parser.add_argument('-w', '--wordlist', type=str, default='/usr/share/wordlists/dirb/common.txt', help='The wordlist that should be used for enumerate, default=common.txt', required=False)
    parser.add_argument('-t', '--target', type=str, help='The target DNS that should be enumerated', required=True)
